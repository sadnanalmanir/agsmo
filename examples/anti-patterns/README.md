...
| **02** | `02_bad_orphan_action.ttl` | `02_good_linked_action.ttl` | Link Goal `--achievedBy-->` Action (and `achieves`) |
| **03** | `03_bad_action_no_performer.ttl` | `03_good_action_with_performer.ttl` | Action needs `performedBy` (+ `performs`) |
...
| **06** | `06_bad_one_sided_link.ttl` | `06_good_dual_edges.ttl` | **AGSMO 0.2:** assert both `hasSubGoal` and `subGoalOf` |

**Note on pair 02:** SHACL does **not** require `achievedBy` (actions can exist without a goal link). The orphan-action case is still a **modelling** anti-pattern for planning/explainability; CI checks the *good* graph passes SHACL and documents the bad narrative in the tutorial.
